...
Set `inspectCauses = true` to match `exceptions` and `skipRetryExceptions` against nested causes as well.
A matching `skipRetryExceptions` entry anywhere in the cause chain prevents retrying, even if another exception matches `exceptions`.
The `condition` closure still receives the original failure. Cause inspection is disabled by default.
